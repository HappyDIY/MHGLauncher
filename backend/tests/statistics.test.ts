...
const resources = { enrich: (value: WishRecord) => value };
...
test("统计保底距离", () => { const value = banner("1", "301", [record("1", 3), record("2", 5), record("3", 3)], resources); expect(value.five_star_items[0]?.pity).toBe(2); expect(value.last_pity).toBe(1); });
test("武器池保底为 80", () => expect(banner("1", "302", [], resources).guarantee_threshold).toBe(80));
test("百分比保留四位", () => expect(banner("1", "301", [record("1", 5), record("2", 3), record("3", 3)], resources).five_star_percent).toBe(0.3333));
test("四星重置紫色保底", () => expect(banner("1", "301", [record("1", 3), record("2", 4), record("3", 3)], resources).last_purple_pity).toBe(1));
