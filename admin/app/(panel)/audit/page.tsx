...
export default async function AuditPage({ searchParams }: { searchParams: Promise<{ cursor?: string }> }) {
  ...
  return <><PageHeader title="审计日志" description="不可修改的云端管理操作记录" />
    <div className="panel table-wrap"><table><thead><tr><th>时间</th><th>站长</th><th>操作</th><th>目标类型</th><th>目标</th><th>结果</th></tr></thead><tbody>{data.items.map((event) => <tr key={event.id}><td><LocalTime value={event.created_at} /></td><td>{event.actor}</td><td className="font-mono text-xs">{event.action}</td><td>{event.target_type}</td><td className="font-mono text-xs">{event.target_ref}</td><td><span className={`badge ${event.result === "success" ? "text-emerald-600" : "text-red-600"}`}>{event.result === "success" ? "成功" : "失败"}</span></td></tr>)}</tbody></table>{!data.items.length && <Empty>暂无审计事件</Empty>}</div>
