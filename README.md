...
脚本会根据代码变化运行必要测试，随后构建并打开应用。所有 App 构建都强制
使用 `packaging/CodeSigning.plist` 中固定的项目统一证书；证书或私钥未安装、
证书指纹不一致、签名校验失败时，构建会立即终止且不会降级为临时签名。
统一证书的私钥只保存在授权构建者的钥匙串中，不提交到仓库。关闭应用后，
成品会保留在：
...
| 应用分发 | 所有构建使用固定的项目统一证书；暂无公证或 DMG 正式发行包 |
