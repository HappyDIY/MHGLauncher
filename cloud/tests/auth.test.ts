...
describe("云端认证工具", () => {
  ...
  test("UID 仅采用官方响应且禁止重定向", async () => {
    const requester = vi.fn().mockResolvedValue(Response.json({ retcode: 0, data: {
	      uid: "100000002", list: [{ uid: "100000002", id: "1", gacha_type: "301", item_id: "", name: "角色", item_type: "角色", rank_type: "5", time: "2026-01-01 08:00:00" }],
    } }));
    const result = await verifyGachaUrl("https://public-operation-hk4e.mihoyo.com/gacha?authkey=x&uid=100000001", requester);
    expect(result.uid).toBe("100000002");
	    const request = requester.mock.calls[0]?.[0] as URL;
