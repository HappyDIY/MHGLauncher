...
export async function verifyGachaUrl(raw: string): Promise<{ uid: string; items: GachaItem[] }> {
  ...
  const payload = await response.json() as { retcode?: number; message?: string; data?: { list?: Record<string, any>[] } };
  if (!response.ok || Number(payload.retcode ?? 0) !== 0) throw new HttpError(422, "gacha_url_expired", payload.message ?? "抽卡 URL 不可用");
  const items = (payload.data?.list ?? []).map((item) => normalize(uid || String(item.uid ?? ""), item));
  const provenUid = uid || items.find((item) => item.uid)?.uid;
  if (!provenUid || items.length === 0) throw new HttpError(422, "gacha_url_unverified", "抽卡 URL 可用，但无法确认 UID");
  return { uid: provenUid, items: items.map((item) => ({ ...item, uid: provenUid })) };
}

export async function issue(uid: string): Promise<{ uid: string; token: string; token_ref: string; reverified_at: string }> {
  ...
  await transaction(async (client) => {
    ...
    await client.query("INSERT INTO sessions(token_hash,uid,reverified_at) VALUES($1,$2,$3)", [hash(token), uid, reverifiedAt]);
  });
...
export async function reverify(token: string, uid: string): Promise<{ uid: string; token: string; token_ref: string; reverified_at: string }> {
  await requireSession(token, uid);
  const reverifiedAt = new Date().toISOString();
  await pool().query("UPDATE sessions SET reverified_at=$1 WHERE token_hash=$2", [reverifiedAt, hash(token)]);
  return { uid, token, token_ref: `keychain:cloud:${uid}`, reverified_at: reverifiedAt };
}

export async function requireSession(token: string, uid?: string): Promise<{ uid: string; reverified_at: string }> {
  const digest = hash(token), result = await pool().query("SELECT uid,reverified_at FROM sessions WHERE token_hash=$1", [digest]);
...
export async function requireFresh(token: string, uid: string): Promise<void> {
  const session = await requireSession(token, uid);
