...
build_test_hash="$(content_signature \
  debug-app.command packaging/Info.plist scripts/build-backend-debug.sh \
  scripts/build-debug-app.sh scripts/build-frontend.sh \
...
force_tests="${MHG_DEBUG_FORCE_TESTS:-0}"

# 测试基线来自上一次成功写入调试 App 的板块签名。
test_baseline=""
for candidate in "$built_app" "$cached_app"; do
  resources="$candidate/Contents/Resources"
  if [[ -f "$resources/.debug-test-frontend-signature" ]] &&
    [[ -f "$resources/.debug-test-backend-signature" ]] &&
    [[ -f "$resources/.debug-test-api-signature" ]] &&
    [[ -f "$resources/.debug-test-build-signature" ]]; then
...
if [[ -n "$test_baseline" ]] && [[ "$force_tests" != "1" ]]; then
  [[ "$(cat "$test_baseline/.debug-test-frontend-signature")" == "$frontend_test_hash" ]] &&
    frontend_changed=0
  [[ "$(cat "$test_baseline/.debug-test-backend-signature")" == "$backend_test_hash" ]] &&
    backend_changed=0
  [[ "$(cat "$test_baseline/.debug-test-api-signature")" == "$api_test_hash" ]] &&
    api_changed=0
  [[ "$(cat "$test_baseline/.debug-test-build-signature")" == "$build_test_hash" ]] &&
    build_changed=0
fi
...
printf '%s\n' "$frontend_test_hash" >"$resources/.debug-test-frontend-signature"
printf '%s\n' "$backend_test_hash" >"$resources/.debug-test-backend-signature"
printf '%s\n' "$api_test_hash" >"$resources/.debug-test-api-signature"
printf '%s\n' "$build_test_hash" >"$resources/.debug-test-build-signature"
